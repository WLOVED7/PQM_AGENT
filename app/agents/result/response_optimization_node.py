...
【关键规则：去重与合并】
- 若多条记录的 inspection_item（检验项）相同，且 specification（规范）内容实质相同或高度相似，
  则合并为一条，并在末尾注明来源文件编号（如"见文件 C37TB-003 至 C37TB-007"）
- 只在 specification 存在实质性差异时才分开列出
- 绝对不要将内容几乎相同的记录重复罗列多次

【示例】
原始: 3条记录，inspection_item 都是"切边"，specification 内容基本一致
优化: **切边**：
- 规范：标准切边 ≥3.0mm；非焊接匹配面边线公差 ±1.0mm
- 检验方法：检具 + 边线规 + 直尺
- 检查频次：首末件，1次/批
（见文件 C37TB-003、004、005）
